...
| **ERC reporting fix** | `run_erc` reported *every* schematic as clean. `parse_erc_json` read a top-level `violations` array — the shape of the *DRC* report — but the ERC report has none: violations are nested under `sheets[]`, and only the enclosing sheet entry carries the sheet name. Violations now also carry KiCAD's machine-readable `type` and their `items[]` (per-item position + uuid), so `run_erc` and `run_drc` return the same shape. Item/position parsing is now one shared helper instead of two copies. | Same class of silent blind spot as the DRC patch above, and worse: a schematic with real errors passed review. Caught by diffing the tool against a raw `kicad-cli sch erc` run — 7 violations reported by the CLI, 0 by the tool. Regression test pins the real nested fixture and asserts it carries no top-level `violations`, so the test can't pass for the wrong reason. |
...
crates/
  ...
  konnect-core         Tool implementations, grouped into toolsets (3 pre-loaded, 15 on-demand)
    ...
    src/tools/cli.rs, verification.rs ← patched DRC + ERC report parsing, .kicad_pro rule fallback (my work)
