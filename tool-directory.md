...
- **Discovery pattern**: the server pre-loads only the **starter kit** (`project`, `config`, `pcb_routing` — 27 tools) so baseline `tools/list` costs ~3.5K tokens instead of ~25K. The LLM reads `list_toolboxes` → calls `load_toolset(name)` to expose additional tools on demand; `unload_toolset(name)` prunes them. `tools/list_changed` is notified on every mutation. If the LLM calls a tool whose toolset isn't loaded, the error names the owning toolset so recovery is a single `load_toolset` hop.
